...
class ACvoltsMMButton extends StatefulWidget {
...
//  final Function onSaveOrUpdate;
  ...
  @override
  createState() => _ACvoltsMMButtonState();
}

class _ACvoltsMMButtonState extends State<ACvoltsMMButton> {
  StreamSubscription<double>? _readingSubscription; // Make it nullable
  Timer? _timer;
  bool isDialogVisible = false;
  Mode idleMode = Mode.idle;

  void _startSubscriptionAndTimer() async {
  ...
  Future<void> acVoltsButtonStartService() async {
    if (isDialogVisible) {
          ...
          child: IconButton(
            ...
            onPressed: () async {
              ...
              if (switchPosition != 'Voltage') {
                ...
                await acVoltsButtonStartService();
              }
            },
          ),
        );
...
/*
class ACvoltsMMButton extends StatefulWidget {
  final Mode selectedMode = Mode.acVoltage;
  final int? selectedRange;
  final TextEditingController? controller;
  final MultimeterService multimeterService;
  final Function(bool) onTimerStatusChanged;
